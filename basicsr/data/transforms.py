...
def paired_random_crop_kd(img_gts_t, img_gts, img_lqs, gt_patch_size_h, gt_patch_size_w, scale, gt_path):
    """Paired random crop.
    ...
    if not isinstance(img_gts_t, list):
        img_gts_t = [img_gts_t]
    if not isinstance(img_gts, list):
        img_gts = [img_gts]
    if not isinstance(img_lqs, list):
    ...
    if len(img_gts_t) == 1:
        img_gts_t = img_gts_t[0]
    if len(img_gts) == 1:
        img_gts = img_gts[0]
    if len(img_lqs) == 1:
        img_lqs = img_lqs[0]
    return img_gts_t, img_gts, img_lqs
